...
export const UpdateStatusSlideShow = createAsyncThunk("slideshow/update-status-slideshow", async (data, thunkAPI) => {
    try {
        return await slideshowService.updateStatusSlideShow(data);
    } catch (err) {
        return thunkAPI.rejectWithValue(err);
    }
})
    ...
    extraReducers: (builder) => {
        builder.addCase(GetSlideShows.pending, (state) => {
            ...
            .addCase(CreateSlideShow.fulfilled, (state, action) => {
                ...
                if (state.isSuccess) {
                    toast.success("The creation of the slideshow was successful!");
                }
            })
            .addCase(CreateSlideShow.rejected, (state, action) => {
                ...
                state.message = action.error;
                if (state.isError) {
                    toast.error("The creation of the slideshow was not successful!");
                }
            ...
            }).addCase(UpdateStatusSlideShow.pending, (state) => {
                state.isLoading = true;
            })
            .addCase(UpdateStatusSlideShow.fulfilled, (state, action) => {
                state.isLoading = false;
                state.isError = false;
                state.isSuccess = true;
                state.updatedslideshow = action.payload;
                if (state.isSuccess) {
                    toast.success("The slideshow update was successful!");
                }
            })
            .addCase(UpdateStatusSlideShow.rejected, (state, action) => {
                state.isLoading = false;
                state.isError = true;
                state.isSuccess = false;
                state.message = action.error;
                if (state.isError) {
                    toast.error("The slideshow update was not successful!");
                }
            })
