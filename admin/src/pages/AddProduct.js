...
const AddProduct = () => {
  ...
  const deleteImg =async (e) => {
    ...
    formik.setFieldValue("images", updatedImages);
    await dispatch(DeleteImg(e.imagePublicId))
    setLoading(false);
  }

  const deleteThumnailImg =async (e) => {
    setLoading(true);
    formik.setFieldValue('thumnailId', "");
    formik.setFieldValue('thumnailUrl', "");
    await dispatch(DeleteImg(e.imagePublicId))
    setLoading(false);
  }
  ...
  const upLoadHinh = async (e) => {
    setLoading(true);
    await dispatch(UploadImg(e))
    setLoading(false);
  }
                    ...
                    formik.values.thumnailId !== "" && formik.values.thumnailUrl !== "" && (
                      <div key={formik.values.thumnailId} className='showImages d-flex flex-wrap gap-3 mb-3 ms-3'>
                        <div className='position-relative'>
                          <button type="button" onClick={() => deleteThumnailImg(formik.values.thumnailId)} className='btn-close position-absolute' style={{ top: "10px", right: "10px" }}></button>
